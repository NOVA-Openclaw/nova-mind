...
|----------|----------|---------|
| `append_run_note(p_run_id integer, p_note text) RETURNS void` | `SECURITY DEFINER`, `search_path=public` pinned. `EXECUTE` granted to all agent roles. | Server-side UTC-timestamped append to `workflow_runs.notes` (nova-mind#557, promotes lesson 757 to enforcement). **This is the write path for run notes** — agents should call `SELECT append_run_note(run_id, 'note text');` rather than `UPDATE workflow_runs SET notes = COALESCE(notes,'')||...` directly. Stamp format: `YYYY-MM-DD HH24:MI UTC — `. Raises on `NULL` note or missing `run_id`; empty-string notes are accepted. **nova-mind#624:** this function was silently removed from `schema.sql`/the live DB by a mechanical schema-dump commit (6b8599e) with no revert trail, then restored. It is now the first (and currently only) entry in `database/.schema-manifest.toml`, which `cognition/scripts/pg-notify-listener.py`'s `sync_schema_to_github()` checks against every fresh dump before committing — a dump that would remove this function by name is refused and reverted rather than silently landing. The listener does **not** check `SECURITY DEFINER`/`search_path` at sync time; those properties are asserted only by the CI backstop `database/check_schema_manifest.py` against the already-committed `schema.sql`. See `CHANGELOG.md`'s `first-install-separate-superuser-624-664` batch for full detail. |

## Schema Sync Safeguards (nova-mind#624)

`database/.schema-manifest.toml` enumerates hand-authored schema objects (currently just `append_run_note(integer, text)`) plus a `[function_security]` assertion for its `SECURITY DEFINER`/`search_path = public` properties that the automated `pgschema dump` → GitHub sync path must never silently remove. Enforced two ways: (1) the listener-side veto in `sync_schema_to_github()` — refuses and reverts the ENTIRE dump if any manifest `[functions]` object is missing by name, before the commit ever lands; (2) `database/check_schema_manifest.py`, a static repo-only CI backstop (no DB connection) wired into `.github/workflows/ci.yml` that additionally asserts the `[function_security]` substrings against the already-committed `schema.sql`, catching the case where `schema.sql` changed via some other path (manual edit, revert, rebase) without the listener ever running. Because the listener does not inspect `SECURITY DEFINER`/`search_path` at sync time, a re-dump that keeps the function name but strips those properties is caught only at the CI layer — a documented commit-to-CI gap. To sanction removing a manifest entry: land a human-reviewed manifest-edit PR first, then perform the live-DB `DROP` separately — never via the listener or any automated path.
