...
> **Status note (post-#320, updated post-#579):** The dedicated `agent_chat`
> database's schema file now grants the full permission matrix (SELECT/INSERT
> plus sequence USAGE/SELECT) for all nova-ecosystem roles as part of schema
> application, satisfying the spirit of this request. As of nova-mind#579, that
> schema file lives at `NOVA-Openclaw/agent-chat`'s `schema.sql` (the
> `database/agent-chat/schema.sql` path referenced below no longer exists in
> this repo), and `register-agent.sh` in that repo now grants per-agent
> permissions automatically (`SELECT, INSERT, UPDATE, DELETE` on both tables,
> `USAGE` on the sequence) as the primary provisioning path. Left as historical
> record of the original ask against the old `nova_memory`-hosted table.
