...
function ensurePgEnv() {
    ...
    try {
        ...
        if (existsSync(pgConfigPath)) {
            ...
            if (config.host)
                process.env.PGHOST = process.env.PGHOST || config.host;
            if (config.port)
                process.env.PGPORT = process.env.PGPORT || String(config.port);
            if (config.database)
                process.env.PGDATABASE = process.env.PGDATABASE || config.database;
            if (config.user)
                process.env.PGUSER = process.env.PGUSER || config.user;
            if (config.password)
                process.env.PGPASSWORD = process.env.PGPASSWORD || config.password;
        }
    }
    catch (err) {
        console.warn("[self-awareness] pg config load failed:", err.message);
    }
}
/**
