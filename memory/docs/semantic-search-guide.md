...
Nova-memory provides sophisticated semantic search capabilities through vector embeddings, combining PostgreSQL's pgvector extension with local embedding models for intelligent content retrieval.

> **Audit note (current as of this pass):** The embedding provider switched from
> OpenAI to **Ollama** in #148/#149. The live system uses `memory/scripts/proactive-recall.py`,
> configured via `memory/scripts/embedding-config.json` (currently `snowflake-arctic-embed2`,
> **1024 dimensions**, `http://localhost:11434`) — not OpenAI's `text-embedding-3-small`
> at 1536 dimensions. The actual `memory_embeddings` table (see `database/schema.sql`)
> has no `metadata JSONB` column. Code blocks below that show OpenAI API calls,
> `VECTOR(1536)`, or a `metadata` column are historical/conceptual illustrations of
> the search patterns involved, not the current literal schema or provider — see
> [semantic-recall.md](semantic-recall.md) for the current, accurate architecture
> description and provider details.
...
"Find conversations about pizza places in Brooklyn"
    ↓
Ollama Embedding API (snowflake-arctic-embed2) → Vector [1024 dimensions]
    ↓  
...
```sql
-- Core embedding table (illustrative — see database/schema.sql for the exact
-- current definition; the live table has no metadata column and uses
-- VECTOR(1024), matching the Ollama snowflake-arctic-embed2 model)
CREATE TABLE memory_embeddings (
    ...
    embedding VECTOR(1024), -- Ollama snowflake-arctic-embed2 embedding (1024 dimensions)
...
> **Audit note (current as of this pass):** The current schema has **no
> per-row `AFTER INSERT` embedding triggers at all** — not for `entity_fact`,
> `event`, `lesson`, or (formerly) `agent_chat`. Verified against
> `/opt/agents/shared-config/nova-memory-schema.sql`: zero `embed_*` trigger
> functions exist. Embeddings are generated by a **batch/cron job**,
> `memory/templates/memory-maintenance.py` (deployed to
> `~/.openclaw/scripts/memory-maintenance.py`), whose Phase 2 ("Embed") embeds
> database rows, research tables, and memory files directly — there are no
> longer separate `embed-full-database.py` / `embed-memories.py` script files;
> `memory-maintenance.py`'s own header says it *replaces* them. The older
> `embed-memories-cron.sh` wrapper is explicitly marked deprecated in its own
> header, pointing callers to `memory-maintenance.py` instead. The
> trigger-based design below is historical/conceptual — it illustrates one way
> automatic embedding *could* work, not how it currently does.
>
> **`agent_chat` specifically:** since the #320 migration moved `agent_chat`
> into its own dedicated database, a same-database trigger referencing
