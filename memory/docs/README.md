...
- [Daily Log Generation](daily-log-generation.md) - Script-generated daily memory log summaries (#397), plus a deterministic completion-log reconcile pass for `work_queue`/`workflow_runs` (see [Completion Log Reconcile](daily-log-generation.md#completion-log-reconcile-nova-mind561), #561)
...
5. **Check for silently-failed extractions:** failed hook invocations (nonzero exit, timeout, spawn error, or JSON-parse failure — nova-mind#497) are captured as dead-letter rows in `extraction_failures` and can be replayed — see the "Failure Handling" section in [memory-extraction-pipeline.md](memory-extraction-pipeline.md#1a-failure-handling-extraction_failures-dead-letter-table--replay-485).
