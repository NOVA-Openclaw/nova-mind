...
> **Note:** `extract-memories.sh` and `store-memories.sh` (the old two-script shell pipeline this diagram used to describe) were removed as part of the #174 grammar-parser removal and consolidated into `memory/scripts/extract_memories.py`. `memory-catchup.sh` previously called a nonexistent `process-input.sh` (a broken no-op path); as of #611 it calls `extract_memories.py` directly, passing the same conversation-context window as the per-turn hook. See `memory/docs/memory-extraction-pipeline.md` for the current pipeline.
