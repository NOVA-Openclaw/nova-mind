 ...
 * Issue: #221
 */
import { getPool } from "./shared/pg-pool.js";
// ── Ollama config ─────────────────────────────────────────────────────────────
...
export default function register(api) {
    ...
    api.on("message_sent", async (event, ctx) => {
        ...
        processMessageSent(event, ctx).catch((err) => {
            console.error("[self-awareness] Unhandled error in message_sent handler:", err instanceof Error ? err.message : String(err));
        });
    });
}
async function processMessageSent(event, ctx) {
    ...
    try {
        ...
        console.warn("[self-awareness] Could not get pg pool:", e.message);
        return;
    }
    // ── Load enabled triggers ─────────────────────────────────────────────────
    let triggers = [];
    try {
        const result = await pool.query(`SELECT id, name, category, keyphrases, keyphrase_embeddings,
       ...
       WHERE enabled = true`);
        ...
        console.warn("[self-awareness] DB query failed:", e.message);
        return;
    }
    ...
    try {
        ...
        console.warn("[self-awareness] Ollama embedding failed for message, skipping:", e.message);
        return;
    }
    ...
    for (const trigger of triggers) {
        ...
        if (trigger.last_triggered_at) {
            const elapsedMinutes = (Date.now() - new Date(trigger.last_triggered_at).getTime()) / 60000;
            if (elapsedMinutes < trigger.cooldown_minutes) {
                console.debug(`[self-awareness] Trigger '${trigger.name}' on cooldown (${elapsedMinutes.toFixed(1)} < ${trigger.cooldown_minutes} min)`);
                continue;
            }
        }
        ...
        if (!embeddings || embeddings.length === 0) {
            try {
                ...
                console.warn(`[self-awareness] Self-heal failed for '${trigger.name}':`, e.message);
                continue;
            }
        }
        if (!embeddings || embeddings.length === 0) {
        ...
        console.info(`[self-awareness] '${trigger.name}' best similarity=${bestSim.toFixed(3)} threshold=${trigger.similarity_threshold}`);
        if (bestSim >= trigger.similarity_threshold) {
            // Fire action
            await fireAction(trigger, content, bestSim, resolvedAgentId, pool);
            // Update trigger stats
            try {
                await pool.query(`UPDATE self_awareness_triggers
           SET last_triggered_at = NOW(), times_triggered = times_triggered + 1
           WHERE id = $1`, [trigger.id]);
            }
            catch (e) {
                console.warn("[self-awareness] Failed to update trigger stats:", e.message);
            }
        }
    }
}
// ── Subagent → parent resolution ──────────────────────────────────────────────
async function resolveSubagentToParent(agentId, pool) {
    try {
        const result = await pool.query(`SELECT p.id AS parent_id, p.name AS parent_name
       FROM agents a
       ...
       LIMIT 1`, [agentId]);
        ...
        console.warn("[self-awareness] Subagent resolution failed:", e.message);
    }
    return agentId;
}
...
async function selfHealKeyphrases(triggerId, keyphrases, pool) {
    ...
    for (const kp of keyphrases) {
        try {
            ...
            console.warn(`[self-awareness] Failed to embed keyphrase '${kp}':`, e.message);
        }
    }
    if (embeddings.length > 0) {
        try {
            await pool.query(`UPDATE self_awareness_triggers SET keyphrase_embeddings = $1::jsonb WHERE id = $2`, [JSON.stringify(embeddings), triggerId]);
        }
        catch (e) {
            console.warn("[self-awareness] Failed to store healed embeddings:", e.message);
        }
    }
    return embeddings;
}
...
// ── Action dispatch ───────────────────────────────────────────────────────────
async function fireAction(trigger, messageContent, similarity, resolvedAgentId, pool) {
    const config = trigger.action_config || {};
    console.info(`[self-awareness] TRIGGER FIRED: '${trigger.name}' ` +
        `category=${trigger.category} sim=${similarity.toFixed(3)} ` +
        `action=${trigger.action}`);
    switch (trigger.action) {
        case "log_only": {
            const logCategory = config.log_category || trigger.category;
