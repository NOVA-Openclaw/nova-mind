...
- **`database/.schema-manifest.toml` + `database/check_schema_manifest.py`** (new files, #624) — enumerate hand-authored schema objects (currently `append_run_note`) that the automated schema-dump sync must never silently remove. `cognition/scripts/pg-notify-listener.py`'s `sync_schema_to_github()` refuses and reverts an entire dump if a manifest object (or its declared `SECURITY DEFINER`/`search_path` properties) is missing; `check_schema_manifest.py` is a second-line static CI check (no DB connection) wired into CI, catching the case where `schema.sql` changed via a non-listener path.
