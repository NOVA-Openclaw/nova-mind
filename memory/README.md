...
# The turn-context plugin replaces old semantic-recall and agent-turn-context hooks
```
...
> **As of nova-mind#320/#579, these tables live in a dedicated `agent_chat`
> database, not `nova_memory`, and the schema/plugin moved to the
> `NOVA-Openclaw/agent-chat` repository.** Agents resolve the connection via the
> nested `agent_chat` section of `~/.openclaw/postgres.json` (see
> `memory/docs/database-config.md`). See the `agent-chat` repo for the full table
> definitions, `send_agent_message()` semantics, and OpenClaw plugin details.
...
For data migrations that must run **before** the schema diff (e.g., renaming a column with backfill), place a `.sql` script in **`database/pre-migrations/`** (repo root — not the unrelated root-level `pre-migrations/` directory, which the installer does not read; see `pre-migrations/README.md`). The installer runs these in filename order before calling `pgschema plan`. Because this step runs *before* the base schema apply, a schema-mutating pre-migration must guard against its target table not existing yet on a fresh DB — wrap the body in `to_regclass('public.<table>') IS NOT NULL` (nova-mind#662) so it no-ops cleanly on first install.

> **Note:** The schema file is generated by `pgschema dump`, not `pg_dump`, and that raw output contains no ownership, privilege, or `\connect` directives. In practice, `database/schema.sql` also carries hand-added `GRANT`/`REVOKE` statements that `pgschema` itself ignores; `agent-install.sh` reconciles those separately after `pgschema apply` (issue #452). See `memory/docs/database-schema-guide.md#schema-management` for details.

## OpenClaw Hooks & Plugin (Automatic Extraction)
...
The `hooks/` directory contains OpenClaw hooks that automatically extract and manage memories. The old **semantic-recall** and **agent-turn-context** hooks have been consolidated into a single **turn-context plugin** using the OpenClaw Plugin SDK.
...
- **turn-context** (Plugin SDK) - Consolidates the old semantic-recall and agent-turn-context hooks into a single plugin running `before_prompt_build`. Performs entity resolution, semantic recall, and turn reminder injection in parallel. [Source](plugins/turn-context/)
...
The hooks and plugin are installed automatically by `agent-install.sh`. To install manually, run the installer:
...
The turn-context plugin registers `before_prompt_build` and runs at prompt construction time.
...
To remove the turn-context plugin:
