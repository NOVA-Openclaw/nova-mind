...
| `extraction_failures` | Dead-letter store for failed memory extractions (#485) — nonzero exit, timeout, spawn error, JSON-parse failure (`json_parse_failure`, #497), or real-time retry exhaustion (`timeout_retries_exhausted`, #680). FK to `channel_transcripts` (`ON DELETE SET NULL`) with a raw-body fallback column. Replayed via `memory/scripts/extraction-replay.sh`. See `memory/docs/memory-extraction-pipeline.md#1a-failure-handling-extraction_failures-dead-letter-table--replay-485`. |
...
Each day's file is no longer purely hand-written. `memory/scripts/generate-daily-log.py` runs from cron (nightly + 3x intraday, installed by `agent-install.sh`) and writes/updates a delimited **generated block** (marked by `<!-- BEGIN GENERATED DAILY LOG -->` / `<!-- END GENERATED DAILY LOG -->` HTML comments) summarizing that day's `agent_chat` activity, `workflow_runs`, `lessons`, `events`, and `tasks` — pulled directly from the database. Agent-written narrative outside the markers is preserved byte-for-byte on every run. A companion cron script, `memory/scripts/completion-log-reconcile.py` (nova-mind#561), runs every 5 minutes and appends completion-side lines (one per closed `work_queue` row or completed `workflow_runs` row) outside the generated block, catching short-lived rows that could otherwise fall between two `generate-daily-log.py` snapshots; both scripts serialize their writes with a shared `flock`. See `memory/docs/daily-log-generation.md` for the full marker contract, flags, cron schedule, and backfill runbook.
...
**Failure handling (#485, #497, #680):** The `memory-extract` hook captures capped (16KB) stderr/stdout tails from the `extract_memories.py` child process, enforces a per-event outer timeout (config-driven via `memory-extraction-config.json`, hardcoded fallback default 95s as of #680 — raised from 90s to keep pace with a new real-time retry loop's worst-case budget; live deployed config currently still sets 90s, see `memory/docs/memory-extraction-pipeline.md`'s "Retry budget vs. outer timeout" note for the resulting margin gap; SIGTERM then SIGKILL after a 5s grace period), and on nonzero exit / timeout / spawn error / JSON-parse failure / retry exhaustion writes a row to the `extraction_failures` dead-letter table rather than discarding the message. Separately, `extract_memories.py` itself now retries transient LLM failures (timeout, connection error, HTTP 429/5xx) up to 3 times in real time before dead-lettering, gated by `EXTRACTION_ENABLE_RETRY` (set only on the hook's real-time spawn path, not by `extraction-replay.sh`) — see "Real-Time Extraction Retry Loop" in `memory/docs/memory-extraction-pipeline.md`. `memory/scripts/extraction-replay.sh` (flock-guarded, rate-limited) replays pending rows, distinguishing `nonzero_exit`/`timeout`/`spawn_error`/`unreplayable`/`json_parse_failure`/`timeout_retries_exhausted` failure reasons (the last two added by migrations `086_extraction_failures_json_parse_failure.sql` (#497) and `database/pre-migrations/007-add-timeout-retries-exhausted-failure-reason.sql` (#680)) and retiring rows to `retry_exhausted` after `EXTRACTION_REPLAY_MAX_RETRIES` (default 5) failed attempts. Full detail: `memory/docs/memory-extraction-pipeline.md`.
