...
### 2026-09-15: First-Install / Separate-Superuser Fix Cluster (#624, #659, #661–#664)

Fixes a cluster of failures affecting a fresh `agent-install.sh` run where the executing DB role is **not** a real PostgreSQL superuser — the common case for a standalone agent install that skips `shell-install.sh`'s interactive superuser prompt (`PG_SUPERUSER` falls back to `DB_USER`, per `lib/pg-env.sh`). Also restores a hand-authored function (`append_run_note()`) that a prior mechanical schema-dump commit had silently clobbered, and adds a durable guard against a repeat.

**What Changed:**
- **`database/schema.sql`** — `append_run_note(integer, text)` restored (#624; was silently removed by commit 6b8599e with no revert trail). 53 `ALTER DEFAULT PRIVILEGES FOR ROLE nova` statements stripped at dump time (#659 phase 1) — those required the executing role to be a superuser or a member of role `nova`, which a fresh non-`nova` install never is; plain `GRANT`/`REVOKE` statements are untouched.
- **`database/.schema-manifest.toml` + `database/check_schema_manifest.py`** (new files, #624) — enumerate hand-authored schema objects (currently `append_run_note`) that the automated schema-dump sync must never silently remove. `cognition/scripts/pg-notify-listener.py`'s `sync_schema_to_github()` refuses and reverts an entire dump if a manifest object is missing by name; it does **not** check `SECURITY DEFINER`/`search_path` at sync time. `check_schema_manifest.py` is a second-line static CI check (no DB connection) wired into CI that additionally asserts those `[function_security]` properties against the already-committed `schema.sql`, catching the case where `schema.sql` changed via a non-listener path and closing the commit-to-CI gap.
- **`agent-install.sh`** early prerequisites check (#661) — warns (non-fatal) if the resolved `PG_SUPERUSER` role is not an actual PostgreSQL superuser, *before* any DB/role work begins, instead of only surfacing as a DDL failure many stages later.
- **`database/pre-migrations/005-backfill-d100-roll-log-announced-at.sql`, `006-lowercase-irc-username-values.sql`** (#662) — guarded with `to_regclass('public.<table>') IS NOT NULL` so they no-op cleanly on a genuinely fresh DB instead of failing with `relation does not exist` (pre-migrations run *before* the base schema apply, so their target tables don't exist yet on first install).
- **`agent-install.sh`** pre-migration step (#663) — each pre-migration file is now copied to a `mktemp`'d `/tmp` path before being handed to the superuser process, mirroring the schema step's existing pattern; needed because pre-migration files live under the agent's `0750` home directory, which a distinct superuser unix user cannot traverse.
- **`agent-install.sh`** pgschema plan/reorder/apply cycle (#664) — intermediate JSON files now live in a dedicated `mktemp -d` scratch directory (mode 777, tracked in a new `TMPDIRS` cleanup array) instead of directly in `/tmp`, whose sticky bit blocked the agent user from renaming a file over one owned by the superuser role. The plan-output file is also no longer pre-created by the agent user, since `pgschema plan` writes it running as the superuser.

**Known non-blocking follow-up (found during QA validation, not fixed in this batch):** `agent-install.sh` currently exits **1**, not 0, on a headless/non-interactive first install — tracked as **nova-mind#667**. Root cause is unrelated to the fixes above: `_install_pg_notify_listener()` runs `systemctl --user daemon-reload` unguarded under `set -euo pipefail`, which hard-kills the installer when there is no user D-Bus session (a common headless-environment condition). All database-side work (schema apply, grant reconciliation, pg-notify-listener file install) completes correctly before this point; only the installer's own exit code is affected. Do not describe or assume a clean exit-0 first-install in headless/non-interactive environments until #667 lands.

See `CHANGELOG.md`'s `first-install-separate-superuser-624-664` batch entry for full detail and `database/schema-reference.md`'s "Schema Sync Safeguards" section for the manifest/veto mechanism.
...
2. **Runs pre-migrations** — executes all `*.sql` files in **`database/pre-migrations/`** (repo root — read via `$SCRIPT_DIR/database/pre-migrations` in `agent-install.sh`) in filename order for any data transformations that must happen before the schema diff. Note: `memory/pre-migrations/` also exists but is empty (just `.gitkeep`) and is not read by the installer — do not confuse it with `database/pre-migrations/`. When `PG_SUPERUSER != DB_USER`, each pre-migration file is first copied to a `mktemp`'d `/tmp` path before being handed to the superuser process (nova-mind#663) — the agent's home directory (`0750`) is not traversable by a distinct superuser unix user. Schema-mutating pre-migrations that reference tables the base schema hasn't created yet must guard themselves with a `to_regclass('public.<table>') IS NOT NULL` existence check so they no-op cleanly on a genuinely fresh DB rather than failing with `relation does not exist` (nova-mind#662 — pre-migrations run *before* the base schema apply, so on first install their target tables may not exist yet).
3. **Plans changes** — runs `pgschema plan` to diff `schema/schema.sql` against the live database, using `--plan-db` pointing at the target DB for accurate extension type resolution (e.g., `vector` from pgvector). The plan/reorder/apply cycle's intermediate JSON files live in a dedicated `mktemp -d` scratch directory (nova-mind#664) rather than directly in `/tmp`, whose sticky bit blocks the agent user from renaming a file over one owned by a separate superuser role; the plan-output file itself is not pre-created by the agent user, since `pgschema plan` writes it running as the superuser.
...
| Property | Details |
|---|---|
| **No SUPERUSER required for most fresh installs (as of nova-mind#659)** | The installer works with a regular, non-superuser database user for the great majority of DDL — `schema.sql` no longer contains the `ALTER DEFAULT PRIVILEGES FOR ROLE nova` statements that used to require the executing role to be a superuser or a member of role `nova` (those were stripped at dump time in #659 phase 1). A superuser may still be needed for extension installation on hosts where the extension isn't already present. `agent-install.sh` runs an early, non-fatal check (nova-mind#661) that warns if the resolved `PG_SUPERUSER` role is not a real PostgreSQL superuser, so a genuine requirement surfaces immediately rather than as a DDL failure many stages later. |
| **`pgschema` itself does not manage privileges** | `pgschema plan`/`pgschema apply` diff and apply structural DDL only — they silently ignore any `GRANT`/`REVOKE` lines present in `schema.sql`. However, `schema.sql` *does* contain hand-maintained `GRANT`/`REVOKE` statements (column- and table-level privileges), and the installer applies those separately: after a successful `pgschema apply`, a post-apply grant-reconciliation step (issue #452) extracts `GRANT`/`REVOKE` lines from the staged schema file and applies them via the superuser connection. So privileges declared in `schema.sql` **are** enforced on install — just not by `pgschema`. |
...
| **Anti-clobber manifest (nova-mind#624)** | `database/.schema-manifest.toml` lists hand-authored schema objects (currently `append_run_note(integer, text)`) that the automated `pgschema dump` → GitHub sync path (`cognition/scripts/pg-notify-listener.py`) must never silently remove; a dump missing a manifest object is refused and reverted before it can be committed. See `database/schema-reference.md`'s "Schema Sync Safeguards" section. |

> **Known non-blocking installer exit-code issue (nova-mind#667):** In headless/non-interactive environments with no user D-Bus session, `agent-install.sh` currently exits **1** even when every DB-side step above completes correctly — `_install_pg_notify_listener()` runs `systemctl --user daemon-reload` unguarded under `set -euo pipefail`, and that single command hard-kills the installer when no D-Bus session exists. This is an installer exit-code defect, not a data-integrity issue; do not assume or document a clean exit-0 first-install in headless environments until #667 is fixed.

### Updating the schema

```bash
# 1. Edit the ROOT database/schema.sql to reflect desired state
#    (memory/schema/schema.sql is reference-only, not authoritative)
...
If a schema change requires a data migration to run first (e.g., rename a column and backfill), place a `.sql` file in **`database/pre-migrations/`** (repo root):
...
> **Do not confuse this with the root-level `pre-migrations/` directory** (see its own
> `pre-migrations/README.md`) — that holds a separate, unrelated set of one-off manual data
> migration scripts (SE Run #27 portfolio schema cleanup) that `agent-install.sh` does
> **not** execute. Only `database/pre-migrations/` is read by the installer.
