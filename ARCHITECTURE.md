...
- **Anti-Clobber Schema Manifest (#624):** `database/.schema-manifest.toml` enumerates hand-authored schema objects (currently `append_run_note(integer, text)`, restored after a prior mechanical `pgschema dump` re-dump commit silently removed it with no revert trail) that the automated dump-and-sync path must never remove. `cognition/scripts/pg-notify-listener.py`'s `sync_schema_to_github()` checks every manifest pattern against each fresh dump before committing, refusing and reverting the entire dump if any listed object (or its declared security properties, e.g. `SECURITY DEFINER`) is missing; `database/check_schema_manifest.py` is a static, DB-connection-free CI backstop wired into `.github/workflows/ci.yml` that catches the case where `schema.sql` changed via a path other than the listener. See `database/schema-reference.md`'s "Schema Sync Safeguards" section.
