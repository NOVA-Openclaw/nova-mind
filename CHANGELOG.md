...
### Batch: motivation-template-gate-check-sync-623 (Issue #623)

#### Changed
- **`motivation/scripts/proactive-gate-check.py`** (nova-mind#623) — Ported three features from NOVA's live nova-workspace `proactive-gate-check.py` into the generic motivation template, keeping the template's existing 11-step cascade (D100 stays at step 11 — no renumbering):
  - **Step 2 tail-review detector** — `_session_tail_flags()` classifies each recent user-facing session's tail (last 8 conversational messages) into `unanswered_user`, `error_in_tail`, and/or `subagent_report`, replacing the prior simple unanswered-message check. `check_step2_unanswered_sessions()` now returns a `by_reason` breakdown alongside the flagged-session list.
  - **`running_workflow_runs`** (new top-level manifest field) — live snapshot of `workflow_runs WHERE status = 'running'`. Computed in `main()` **before** the idle short-circuit, so it is present in both the abbreviated non-idle manifest and the full idle manifest. On DB failure returns its error dict **verbatim** (never falls back to `[]`) — an empty list means "queried successfully, nothing running," while an error dict means "the check itself failed," and the two must stay distinguishable. This is the subtlest part of the change and the one most likely to regress under a careless refactor.
  - **`top_active_channels`** (new top-level manifest field) — topic-aware ranking of the busiest user-facing channels (default top 6) by non-self message volume, for heartbeat introspection to read transcripts directly (I)ruid directive, 2026-08-29).
  - **Excluded by design (not ported, do not "restore"):** `check_step7_social_engagement` and `check_step13_watchdog_triage` are NOVA-specific gates whose producer (`file-watchdog-recovery-task.py`) lives only in nova-workspace, not nova-mind. The generic template intentionally keeps its 11-step cascade rather than adopting the live script's 11/13/12 ordering artifact. Also removed two leftover `'[watchdog'`/`'watchdog-recovery'` string literals that had slipped into `_SUBAGENT_REPORT_MARKERS` during the initial port.
  - **Bug fixes surfaced by the QA suite:** `_iso_from_db()`'s non-datetime defensive fallback previously returned a raw `datetime` instead of a formatted ISO string; `_text_looks_intentional_silence()` now recognizes underscore-retaining token forms (e.g. `"  NO_REPLY.  "`) as intentional silence.
  - Test suite: 241 passed / 0 failed (previously 221 passed / 20 failed).
- **`motivation/README.md`, `motivation/ARCHITECTURE.md`, `motivation/scripts/README.md`** (nova-mind#623, this batch) — documented the three ported features above, with particular emphasis on the `running_workflow_runs` error-passthrough contract (see `motivation/ARCHITECTURE.md#running_workflow_runs-manifest-field`).
