...
### Batch: first-install-separate-superuser-624-664 (Issues #624, #659, #661, #662, #663, #664)

SE Run #804. Closes the first-install / separate-superuser failure cluster: a fresh `agent-install.sh` run where the executing DB role is not a real PostgreSQL superuser (the common case for a standalone agent install that skips `shell-install.sh`'s interactive superuser prompt) previously failed at several different DDL stages with no clear diagnostic tying the failures together. Also restores a hand-authored function that a prior mechanical schema-dump commit silently clobbered, and adds a durable guard against a repeat.

#### Fixed
- **#624 — `append_run_note()` restored to `schema.sql` and the live `nova_memory` database** — Commit 6b8599e (2026-08-09), a mechanical `pgschema dump` re-dump, had silently removed the hand-authored `append_run_note(integer, text)` function (added in #557/#560) from `database/schema.sql` with no revert message and no changelog entry; the function was also absent from the live production database. Restored the exact `SECURITY DEFINER`, `SET search_path = public` definition to both `database/schema.sql` and the live DB. See `database/schema-reference.md`'s Functions table for the current signature and usage.
- **#624 — Anti-clobber veto so this cannot recur silently** — `database/.schema-manifest.toml` (new file) enumerates hand-authored schema objects (`append_run_note(integer, text)` is the first entry, plus a `[function_security]` section asserting its `SECURITY DEFINER`/`search_path` properties as literal substrings) that automated schema-sync tooling must never remove. `cognition/scripts/pg-notify-listener.py`'s `sync_schema_to_github()` now checks every manifest `[functions]` pattern against a freshly-dumped `schema.sql` **before** `git add`/commit/push; if any manifest object is missing by name, the entire sync is refused, the dump is reverted (`git checkout -- database/schema.sql`), and a distinct `agent_chat` alert names the missing object(s). A dump that removes a *non-manifest* object is unaffected and proceeds normally — this is a targeted veto, not a general anti-drop lock. `database/check_schema_manifest.py` (new file, wired into `.github/workflows/ci.yml`) is a second-line static CI backstop that additionally asserts the `[function_security]` substrings against the already-committed `schema.sql` with no DB connection required, fails **closed** (nonzero exit) on a malformed manifest, and catches the case where `schema.sql` changed through a path other than the listener (manual edit, revert, rebase). Because the listener does not inspect `SECURITY DEFINER`/`search_path` at sync time, a re-dump that keeps the function name but strips those properties is caught only at the CI layer — a documented commit-to-CI gap. Sanctioned removal path for a manifest entry: a human-reviewed PR editing the manifest first, then a separate live-DB `DROP` — never via the listener or any automated path. Manifest header documents the recurring mechanical-dump-clobber failure class this closes (five prior incidents: 50c1df3/7f979dc/11b00eb, e1ede21, 6b427e5, in addition to 6b8599e/#624 itself).
- **#659 (phase 1) — `ALTER DEFAULT PRIVILEGES FOR ROLE nova` statements stripped from `schema.sql` at dump time** — `schema.sql` shipped 53 `ALTER DEFAULT PRIVILEGES ... FOR ROLE nova ...` statements hardcoding the production owner role (`nova`) and NOVA's own subagent roster as grantees. `FOR ROLE nova` requires the executing role to be a superuser or a member of role `nova` — a condition a fresh non-`nova` agent install never satisfies, and the direct root cause of #661. `sync_schema_to_github()` now dumps to an in-memory buffer, strips ADP (`Type: DEFAULT_PRIVILEGE`) blocks via a new `_strip_default_privileges()`, and only then writes `schema.sql` — running before the #624 manifest veto so the veto evaluates the same artifact that gets committed. The currently-checked-in `database/schema.sql` received the same strip (53 blocks / 585 lines removed, pure deletion, `append_run_note()` presence unaffected). **Scope: phase 1 only** — plain `GRANT`/`REVOKE` statements (`Type: PRIVILEGE`/`COLUMN_PRIVILEGE`) are untouched; the #452 post-apply grant-reconciliation step in `agent-install.sh` still depends on those lines being present. Verified: `pgschema plan`+`apply` of the ADP-stripped schema succeeds end-to-end against a fresh scratch DB owned by a non-superuser, non-`nova` role.
- **#661 — Early guard warns when `PG_SUPERUSER` is not a real superuser** — Standalone `agent-install.sh` runs (no `shell-install.sh` prompt) leave `PG_SUPERUSER` unset, and `lib/pg-env.sh`'s fallback resolves it to the agent's own `DB_USER`. Previously this only surfaced later, at DDL time, as a `42501` failure on the ADP statements #659 has since stripped — database creation succeeded first, splitting the failure from its cause by several install stages. A new check in the prerequisites section (before any DB/role work) queries `pg_roles` for the resolved `PG_SUPERUSER` role and warns loudly — but does **not** abort — if it exists but is not a real superuser (`rolsuper = false`), or if the role does not exist yet. Non-fatal by design: #659 phase 1 means most fresh installs now succeed without a real superuser at all, so failing closed here would block installs that no longer need superuser privilege. The guard checks only `rolsuper` (the one DDL-privilege requirement that can still legitimately need a real superuser going forward, e.g. `CREATE EXTENSION`) — an earlier draft also checked `nova`-role membership, but that mirrored the ADP requirement #659 already removed, so checking it would test a requirement the guard's own trigger condition no longer needs.
- **#662 — Schema-mutating pre-migrations guarded against running before their target tables exist** — Pre-migrations run *before* the base schema apply in `agent-install.sh`. On a fresh first-install DB, `database/pre-migrations/005-backfill-d100-roll-log-announced-at.sql` and `006-lowercase-irc-username-values.sql` referenced `d100_roll_log`/`entity_facts` — tables that do not exist yet on a truly empty DB — and failed with `relation does not exist`. Invisible to repo-checkout testing, which always runs against an already-populated DB. Both files are now wrapped in a `to_regclass('public.<table>') IS NOT NULL` existence guard (chosen over reordering: lower blast radius, and on a genuinely fresh DB both migrations are semantic no-ops anyway since no rows exist yet to backfill/lowercase). Verified directly: both files run clean (exit 0, no-op) against a freshly created empty scratch DB, and still perform their real work against the populated `nova_memory` DB where the target tables exist.
- **#663 — Pre-migration files copied to `/tmp` so a separate superuser can read them** — When `PG_SUPERUSER != DB_USER`, pre-migration files under the agent's `$HOME` (mode `0750`) were not traversable by the distinct superuser unix user running the DDL. Mirrors the schema step's existing `/tmp`-copy + `chmod 644` pattern: each pre-migration file is copied to a `mktemp`'d `/tmp/pgschema-premigration-XXXXXX.sql` (tracked in `TMPFILES` for cleanup) before being passed to `_superuser_psql`.
- **#664 — `pgschema` plan/reorder/apply scratch files no longer pre-created by the wrong user** — `/tmp` has the sticky bit (`1777`), which blocks the agent user from renaming a file over a file owned by the `postgres`/superuser role — a failure mode hit by the plan → reorder → apply cycle's intermediate JSON files. Fixed in three steps: (1) initially made the plan/reorder output files superuser-writable; (2) switched to a dedicated non-sticky scratch directory (`mktemp -d /tmp/pgschema-scratch-XXXXXX`, `chmod 777`, tracked in a new `TMPDIRS` array cleaned up alongside `TMPFILES`) so both the agent user and the superuser can create/rename files inside it; (3) stopped pre-creating the plan-output file entirely — since `pgschema plan` writes it running *as* the superuser, the agent user must not create the inode first, or the superuser's later write conflicts with an agent-owned file. `cleanup_tmp()` now removes both tracked temp files and tracked temp directories.

#### Known non-blocking follow-up (found during QA validation of this batch, not fixed here)
- **#667 — `agent-install.sh` exits 1 in headless/non-interactive environments even on a successful install** — `_install_pg_notify_listener()` runs `systemctl --user daemon-reload` unguarded, under the script's `set -euo pipefail`. In a headless environment with no user D-Bus session (`Failed to connect to bus: No medium found`), that single line hard-kills the entire installer with exit 1 — even though every DB-side step (schema apply, grant reconciliation, pg-notify-listener file install) completed correctly ahead of it. Discovered during SE Run #804 Step 8 QA validation of this exact batch on a real separate-superuser first install at commit b7784e9. **This means: a headless/non-interactive `agent-install.sh` invocation on this branch does not yet exit 0 on success** — do not describe or rely on a clean exit-0 first-install until #667 lands. Sibling of #379 (same `systemctl --user` D-Bus dependency, different call site — service enable/start under `sudo` rather than `daemon-reload` under a bare headless shell).

#### Issues Closed
- #624 — `append_run_note()` silently removed by a mechanical schema-dump commit, no revert/changelog trail
- #659 (phase 1) — `ALTER DEFAULT PRIVILEGES FOR ROLE nova` in `schema.sql` requires superuser/role-nova membership, breaking fresh non-`nova` installs
- #661 — no early diagnostic when `PG_SUPERUSER` resolves to a non-superuser role; failure only surfaced later at a DDL step
- #662 — schema-mutating pre-migrations 005/006 fail with "relation does not exist" on a fresh first-install DB
- #663 — pre-migration files unreadable by a separate superuser unix user due to `0750` agent home permissions
- #664 — `pgschema` plan/reorder/apply scratch files hit `/tmp`'s sticky-bit rename restriction between the agent user and a separate superuser
