...
All five subsystems share a single PostgreSQL database (`{username}_memory`) and a unified installer. **Exception (nova-mind#320/#579):** inter-agent messaging (`agent_chat`/`agent_chat_processed`) was moved out of `{username}_memory` into its own dedicated `agent_chat` database, shared across agents, to remove the per-agent replication that previously kept messaging in sync across separate memory databases. As of nova-mind#579, the bus schema, migrations, plugin, and installer live in the dedicated `NOVA-Openclaw/agent-chat` repository; nova-mind detects an optional peer bus and delegates registration/plugin setup to that checkout. Agents resolve the `agent_chat` connection via a nested section of `~/.openclaw/postgres.json` rather than the flat top-level keys used for `{username}_memory`. See `memory/docs/database-config.md` and the `agent-chat` repo for details.
