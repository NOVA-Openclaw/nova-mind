-- Final schema tests for nova-mind issue #474.
--
-- Covers Area 1 (TC-474-01..09), TC-474-18 (grants), and TC-474-40/36/37/38
-- (resolve_entity_by_identifier). Run against a disposable database with
-- database/schema.sql already applied, as a user with table-owner privileges
-- (e.g. nova).
--
-- Usage:
--   createdb issue474_test_schema
--   psql -U nova -d issue474_test_schema -f database/schema.sql
--   psql -U nova -d issue474_test_schema -f tests/TEST-474-schema.sql
...
-- Schema.sql's ALTER DEFAULT PRIVILEGES block requires a superuser, so this
-- test simulates the production grants, re-applies the intended REVOKEs, and
-- then grants the test user DML for constraint/function exercises.
DO $$
DECLARE
    v_role text;
...
-- Sequences must be usable by the writer-of-record in the fixture DB.
GRANT USAGE, SELECT ON SEQUENCE comms_items_id_seq, comms_responses_id_seq TO hermes;
GRANT ALL ON SEQUENCE comms_items_id_seq, comms_responses_id_seq TO CURRENT_USER;
...
-- ── TC-474-18: grants for writer-of-record ──────────────────────────────────
DO $$
DECLARE
...
-- Grant test user DML for constraint/function tests.
...
BEGIN
    ...
    VALUES ('__test_entity_474_' || gen_random_uuid()::text, 'person')
...
BEGIN
    ...
    RAISE NOTICE '%', _test_result('TC-474-06 COMMENTs',
        ...
        AND v_table_comment ILIKE '%communications%',
...
-- ── TC-474-07: no updated_at column (design decision C) ─────────────────────
DO $$
DECLARE
...
-- Clean up deterministic test rows.
