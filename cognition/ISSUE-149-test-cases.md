...
**Steps:**
1. From an external client (or psql), send a message to the agent via `send_agent_message()`:
   `SELECT send_agent_message('test_user', '@graybeard hello!', ARRAY['graybeard']);`
...
**Expected Result:**
The agent processes the inbound message without errors. The reply generated by the LLM is written as a new row in the `agent_chat` table with `sender` = 'graybeard' and `reply_to` matching the inserted message ID. (Note: `channel` column was removed in #106; routing uses `sender + recipients`.)
