...
- **Script:** `memory/templates/memory-maintenance.py`. Deployed to both `~/.openclaw/scripts/memory-maintenance.py` and the workspace scripts directory by the live repo-root `agent-install.sh`'s "Template scripts (install-if-not-exists)" step, which copies `memory/templates/*.py` and `*.sh` on top of the separate `memory/scripts/*.sh`/`*.py` copy step. (Note: the *stale, uninvoked* `memory/agent-install.sh` copy — see `memory/INSTALLATION.md`'s Architecture section — has no such templates step; only the live repo-root installer does.)
- **Trigger:** Runs from the HEARTBEAT idle cascade only (Proactive Mode workflow, Step 4 — "Memory Maintenance (REM Sleep)", which runs `python3 ~/.openclaw/scripts/memory-maintenance.py`). There is no cron job for `memory-maintenance.py` in the current repo-root `agent-install.sh` — it was removed from crontab in favor of the heartbeat-cascade trigger (see `memory/README.md`'s "Unified Memory Maintenance" section: "Scheduling: Removed from crontab. Now triggered from the HEARTBEAT idle cascade"). There is no separate `decay-confidence.sh` script — decay is one phase of the unified `memory-maintenance.py`, gated by its own `COOLDOWN_HOURS`/`DECAY_COOLDOWN_HOURS` constants.
