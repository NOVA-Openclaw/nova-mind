...
COMMENT ON FUNCTION get_agent_bootstrap IS 'Get all bootstrap files for an agent: UNIVERSAL + GLOBAL + DOMAIN-matched + WORKFLOW summaries (matched by orchestrator_domain or workflow_steps.domain/domains[] overlap with agent_domains) + AGENT-specific. Domain-based workflow matching (orchestrator_agent_id and workflow_steps.agent_id were dropped). See nova-mind#171 / 5b20b40.';
