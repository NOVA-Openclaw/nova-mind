...
>
> **Second post-implementation note (nova-mind#660, added at SE run #866 Step 9):** F10 is now stale for `sync.ts` — as of #660, `buildAgentsList()` **does** map a valid `thinking` value to an emitted `thinkingDefault` key (the "not a valid per-agent config key" comment F10 cites was removed as part of that fix). F10 remains accurate **only** for `agent-install.sh`'s inline `_generate_agents_json()` query, which was not updated alongside the #660 `sync.ts` change and still never emits a `thinkingDefault`/`thinking` key — see `TC-429-P-06` below, which is consequently also stale as an installer/plugin *parity* claim (the two code paths no longer agree on this field) even though it remains true of the installer path in isolation.
...
| F10 | *(#429 design-time fact, no longer current for `sync.ts` — see post-implementation note below)* `sync.ts` never emits the `thinking` key (explicit comment: not a valid per-agent config key; set at spawn time). The installer's ported query likewise never emits a `thinking` key even though `get_agent_export_rows()` returns a `thinking` column — confirmed still true post-#429-fix. | sync.ts comment; agent-install.sh:2374-2450 |
...
- **Expected (design-time / #429):** Neither installer JSON nor plugin JSON contains a `thinking` key anywhere in the entry (F10). Guards against an over-eager port that naively maps every returned column to a JSON key.
- **STALE as of nova-mind#660 (added SE run #866 Step 9):** This is no longer true for the plugin path. Post-#660, a valid `thinking` value **does** produce a `thinkingDefault` key via `buildAgentsList()`. The installer's inline query was not updated, so the two code paths currently **diverge** on this field — a real parity gap, not just a stale test. See this run's doc-audit report (`reports/`) for detail; do not use this test case as written to validate current behavior.
