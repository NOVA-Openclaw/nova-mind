...
- **Declarative Schema Management:** Uses `pgschema` to diff `schema.sql` against live database; applies only needed changes. Between `pgschema plan` and `pgschema apply`, the installer runs the plan JSON through `database/plan_reorder.py` (nova-mind#597), a dependency-aware reorderer that parses each planned statement with `pglast`, builds an object-level dependency graph (tables, columns, types, functions, views — including alias-resolved view→column references and materialized views — plus metadata dependents such as `COMMENT`/`GRANT`/`ALTER OWNER`/`SECURITY LABEL`, which are reassigned to follow the `CREATE` they depend on), and emits a topologically-sorted plan. This closes a class of fresh-install failures (#597, #447, #392) where `pgschema`'s own plan ordering placed a dependent statement (e.g. a `GRANT` or a view referencing a not-yet-added column) before the statement that creates its dependency, aborting the entire transaction group. Reorder failure (malformed plan, unparseable SQL, or a true dependency cycle — exit codes 2/3/4 respectively) aborts the install with a statement-level diagnostic; a `null`/absent/empty `groups` array (the shape `pgschema` emits for a true no-op plan) passes through unchanged so idempotent re-installs are unaffected. Known limitations (CTE column dependencies, `ADD COLUMN ... DEFAULT` expression refs, cross-table index predicates, `DROP TYPE` identity, function-overload disambiguation) are tracked in nova-mind#600. `pgschema` deliberately ignores privilege statements (`GRANT`/`REVOKE`), so a dedicated post-apply reconciliation step (issue #452) extracts `GRANT`/`REVOKE` lines from the staged schema file and re-applies them via the superuser connection after a successful `pgschema apply` — otherwise explicit grants added directly to `schema.sql` are silently lost on fresh installs.
- **Schema-Apply Exit-Code Gate (#597):** Prior to this batch, a failed `pgschema apply` printed a warning (`Schema apply failed (exit N) — continuing`) but the installer still exited 0, so CI/deploy pipelines could not detect the failure without grepping logs. The installer now hard-fails (`exit 1`) on any unrecovered schema-apply-stage error (extension install, pre-migration, renames, plan, reorder, apply, or grant reconciliation) — the only stage that intentionally exits 0 on skip is the destructive-changes hazard check, which is a deliberate safety stop, not a failure.
- **Separate-Superuser First-Install Support (#624, #659, #661–#664):** A fresh install where the executing DB role is not a real PostgreSQL superuser (`PG_SUPERUSER` falling back to `DB_USER`, the common case for a standalone agent install that skips `shell-install.sh`'s interactive superuser prompt) is now the expected path, not a failure mode. `schema.sql` no longer contains `ALTER DEFAULT PRIVILEGES FOR ROLE nova` statements that required superuser/role-nova membership (#659, stripped at dump time); an early prerequisites check warns — non-fatally — if the resolved `PG_SUPERUSER` is not a real superuser, before any DB/role work begins (#661); pre-migration files are copied to `/tmp` so a separate superuser unix user can read them past the agent's `0750` home permissions (#663); schema-mutating pre-migrations are guarded with `to_regclass(...) IS NOT NULL` so they no-op on a genuinely empty fresh DB instead of failing (#662); and the `pgschema` plan/reorder/apply cycle uses a dedicated non-sticky `mktemp -d` scratch directory instead of `/tmp` directly, avoiding a sticky-bit rename conflict between the agent user and the superuser role (#664). **Known non-blocking exception (#667):** headless/non-interactive invocations still exit 1 due to an unguarded `systemctl --user daemon-reload` in the pg-notify-listener install step (no user D-Bus session) — all DB-side work completes correctly before this point; only the installer's own exit code is affected.
- **Anti-Clobber Schema Manifest (#624):** `database/.schema-manifest.toml` enumerates hand-authored schema objects (currently `append_run_note(integer, text)`, restored after a prior mechanical `pgschema dump` re-dump commit silently removed it with no revert trail) that the automated dump-and-sync path must never remove. `cognition/scripts/pg-notify-listener.py`'s `sync_schema_to_github()` checks every manifest `[functions]` pattern against each fresh dump before committing, refusing and reverting the entire dump if any listed object is missing by name. It does **not** check `SECURITY DEFINER`/`search_path` at sync time — a dump that keeps the function name but drops those properties is not caught live. That gap is closed by `database/check_schema_manifest.py`, a static, DB-connection-free CI backstop wired into `.github/workflows/ci.yml` that additionally asserts `[function_security]` substrings against the already-committed `schema.sql`. See `database/schema-reference.md`'s "Schema Sync Safeguards" section.
